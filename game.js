...
let gameLoopId = null, score = 0, isGameOver = false, isStarted = false, currentGameMode = 'infinite';
...
window.startGame = (mode, mapId) => {
    ...
    cameraX = 0; deadlineX = -600; lastSpawnX = 1200; particles = []; guideOpacity = 1;
    hook.active = false;
    
    canvas.width = window.innerWidth; canvas.height = window.innerHeight;

    if (mode === 'speedrun') {
        obstacles = [...map1.obstacles];
        document.getElementById('mode-label').innerText = "SPEEDRUN: MAP1";
...
function update() {
    ...
    if (currentGameMode === 'infinite') {
        ...
        if (player.x / 10 >= map1.goalDist) finishGame(true); 
    }
...
function draw() {
    ...
    if (currentGameMode === 'infinite') {
        ...
        ctx.moveTo(wallX, 0); ctx.lineTo(wallX, canvas.height); ctx.stroke();
        ctx.restore();
    }

    // 2. 조준선 레이저 그리기 (독립적 경로)
    if (isStarted && !isGameOver && !hook.active) {
        const sx_world = player.x + 11;
        const sy_world = player.y + 11;
        const ang = Math.atan2(mousePos.y - sy_world, (mousePos.x + cameraX) - sx_world);
        const tx_world = sx_world + Math.cos(ang) * hook.maxDist;
        const ty_world = sy_world + Math.sin(ang) * hook.maxDist;
        ...
        obstacles.forEach(o => {
            ...
            if (tmax >= tmin && tmax > 0 && tmin < 1 && tmin < minT) { 
                minT = tmin; canHit = true; 
            }
        });
        ...
        ctx.beginPath();
        ctx.setLineDash([8, 6]);
        ctx.strokeStyle = canHit ? 'rgba(255, 255, 0, 0.6)' : 'rgba(255, 0, 0, 0.6)';
        ctx.lineWidth = 2.5;
        ctx.moveTo(sx_world - cameraX, sy_world);
        ctx.lineTo(tx_world - cameraX, ty_world);
        ctx.stroke();
        ctx.restore();
    }
    ...
    obstacles.forEach(o => {
        ...
        ctx.strokeRect(o.x - cameraX, o.y, o.w, o.h);
        ctx.restore();
    });
    ...
    if (hook.active && player.alive) {
        ctx.save();
        ctx.beginPath();
        ctx.shadowBlur = 10; ctx.shadowColor = '#ffff00'; ctx.strokeStyle = '#ffff00'; ctx.lineWidth = 3;
        ctx.moveTo(player.x+11-cameraX, player.y+11); ctx.lineTo(hook.x-cameraX, hook.y); ctx.stroke();
        ctx.restore();
    }
...
function finishGame(isWin) {
    ...
    if (currentGameMode === 'infinite') {
        title.innerText = "SYSTEM CRASHED";
...
function loadRanking() {
    const path = `scores/${currentGameMode}`;
    const isSpeedrun = (currentGameMode === 'speedrun');
    const scoresRef = isSpeedrun 
        ? query(ref(db, path), orderByChild('score'), limitToFirst(10))
        : query(ref(db, path), orderByChild('score'), limitToLast(10));

    onValue(scoresRef, (snapshot) => {
        const data = snapshot.val();
        let sorted = data ? Object.values(data) : [];
        ...
        document.getElementById('rankList').innerHTML = sorted.map((s, i) => 
            `<li><span>${i+1}. ${s.name}</span> <span>${s.score}${isSpeedrun ? 's' : 'm'}</span></li>`).join('');
    });
}
...
window.addEventListener('keydown', e => {
    ...
    if (isGameOver && canReboot && e.code === 'Space') {
        window.startGame(currentGameMode, 'map1');
    }
});
...
window.addEventListener('mousedown', e => {
    ...
    if (hit) {
        ...
        hook.length = Math.sqrt((hit.x-sx)**2 + (hit.y-sy)**2);
...
document.getElementById('submitBtn').onclick = () => {
    ...
    const path = `scores/${currentGameMode}`;
    set(push(ref(db, path)), { name, score: parseFloat(score) }).then(() => {
        document.getElementById('nameInputArea').innerHTML = "DATA UPLOADED";
    });
};